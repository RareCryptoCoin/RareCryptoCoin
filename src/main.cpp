...
#if defined(NDEBUG)
# error "Rarecryptocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x59fff039d26cb6ec7e0636b71458ea41b83d32d69220e5d955485658658c348b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rarecryptocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rarecryptocoin Signed Message:\n";
...
{
    // Rarecryptocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rarecryptocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rarecryptocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1504252812, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Herald 30th of august The tides are changing";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0423b8ce27213ff6a2d440ff1a4e06478bf87a54d11532b9570e80273a119fd4f6bee586bf5c66c6280b02273d77248c7f959f5bfdaa6d068007c1831096020eb7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504252812;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3330239;
        ...
        assert(block.hashMerkleRoot == uint256("0xee113eb9644088f65a899763a8deb1c5f7ab3f89aee8df7b178c06c19f1b6f30"));
...
unsigned char pchMessageStart[4] = { 0x3e, 0xe1, 0xab, 0x05 }; // Rarecryptocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RarecryptocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RarecryptocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RarecryptocoinMiner(CWallet *pwallet)
{
    printf("RarecryptocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rarecryptocoin-miner");
    ...
    try { loop {
        ...
        printf("Running RarecryptocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RarecryptocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RarecryptocoinMiner, pwallet));
}
